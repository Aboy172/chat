 ...
 * @description: 私聊消息结果处理工作类
 * @version: 0.0.1
 **/
@RedisMQListener(queue = ChatRedisKey.CHAT_RESULT_PRIVATE_QUEUE, batchSize = 100)
@Component
@RequiredArgsConstructor
public class PrivateMessageResultTask extends AbstractMessageResultTask<ChatSendResult> {
    private final MessageListenerMulticaster messageListenerMulticaster;

    @Override
    public void onMessage(List<ChatSendResult> data) {
        messageListenerMulticaster.multicast(ChatListenerType.PRIVATE_MESSAGE, data);
    }
